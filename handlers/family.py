...
async def start_create_family(message: types.Message, state: FSMContext):
    # Перевіряємо, чи користувач вже в сім'ї
    fid = db.get_user_family(message.from_user.id)
    if fid:
        await message.answer("Ви вже в сім'ї! Спочатку покиньте поточну сім'ю.")
        return
...
async def process_family_name(message: types.Message, state: FSMContext):
    try:
        ...
        await state.clear()
        await message.answer(f"Помилка при створенні сім'ї: {str(e)}. Спробуйте ще раз або зв'яжіться з адміністратором.")
