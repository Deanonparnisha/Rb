...
class Database:
    ...
    def create_family(self, user_id, family_name):
        ...
        with self.connection:
            # Перевіряємо, чи користувач вже має сім'ю
            old_family_id = self.cursor.execute("SELECT family_id FROM users WHERE user_id = ?", (user_id,)).fetchone()
            if old_family_id and old_family_id[0]:
                # Якщо користувач вже в сім'ї, спочатку виходимо з неї
                self.leave_family(user_id)
            ...
            # 2. Створюємо запис в resourses.db (використовуємо INSERT OR IGNORE для безпеки)
            self.cursor.execute("INSERT OR IGNORE INTO res.storage (family_id) VALUES (?)", (family_id,))
